...
var XListItem = Class.create({
        initialize: function(content, options) {
          ...
          this.containerElement = PElement("div", {'class': 'xitemcontainer'}).insert(content || '');
          this.containerElement._p_addClassName(this.options.containerClasses || '');
          this.containerElement._p_setStyle({textIndent: '0px'});
          if (this.options.value) {
            this.containerElement._p_insert(PElement('div', {'class':'hidden value'}).insert(this.options.value));
          }
          this.listItemElement = PElement("li", {'class' : classes}).update( this.containerElement );
          if (this.options.icon && !this.options.icon.blank()) {
